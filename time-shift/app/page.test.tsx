import { timeSort, unixFormat, TimeShift } from "./page"
...
describe('timeSort function', () => {
    ...
    test('sorts by duration from longest to shortest', () => {
    ...
    test('sorts by startTime in ascending order (asc)', () => {
        const sortedAsc = timeSort(timeArr, 'asc');
        
        // Verify the order of shifts based on ascending startTime
        expect(sortedAsc[0].startTime).toBeLessThanOrEqual(sortedAsc[1].startTime);
        expect(sortedAsc[1].startTime).toBeLessThanOrEqual(sortedAsc[2].startTime);
