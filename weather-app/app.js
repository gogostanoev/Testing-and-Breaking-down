...
function currentInfoCity(info){
  ...
  <div>Highest humidity: ${maxHumInfo(info.list)} &deg;C</br>
  Avg humidity: ${averageHumInfo(info.list)} &deg;C </br>
  Min humidity: ${minHumInfo(info.list)} &deg;C </div>`;
...
function averageTempInfo(arrayOfAvgTemps){
  ...
  for(let i = 0; i < arrayOfAvgTemps.length; i++) {
    averageTemp += arrayOfAvgTemps[0].main.temp
  }

  averageTemp = averageTemp / arrayOfAvgTemps[0].main.temp
...
function averageHumInfo(arrayOfAvgHum){
  ...
  for(let i = 0; i < arrayOfAvgHum.length; i++){
    averageHum += arrayOfAvgHum[0].main.humidity
  }

  averageHum = averageHum / arrayOfAvgHum[0].main.humidity
